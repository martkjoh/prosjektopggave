...
\end{equation}
To incorporate a finite isospin density, we have to change the parametrization of $\SU(2)_A$.
We follow the analysis in~\cite{Andersen:two-flavor-chpt}.
We ssume the groundstate is independent of space, $\pi_a(x) = \pi_a"0$, and write it as
\begin{equation}
    \Sigma_\alpha 
    :=
...
\begin{equation}
    \nabla_\mu \Sigma_\alpha = - v^a_\mu [\tau_a, \Sigma_\alpha].
\end{equation}
...
\begin{equation}
    ...
    \right]
\end{equation}
For $\mu_I = 0$, this is minimized by $\alpha = 0$.
...
\begin{equation}
    \Sigma(x) = R_0(x) \Sigma_0 L_0^\dagger(x) = U(x) \Sigma_0 U(x).
\end{equation}
where
\begin{equation}
...
\begin{equation}
    ...
    = \hat L \Sigma_0 \hat R^\dagger = A_\alpha \Sigma_0 A_\alpha.
\end{equation}
where
\begin{equation}
...
\begin{align}
    L_0 & \rightarrow L_\alpha = \hat L L \hat L^\dagger = A_\alpha L A_\alpha^\dagger, \\
    R_0 & \rightarrow L_\alpha = \hat R R \hat R^\dagger = A_\alpha^\dagger L A_\alpha.
\end{align}
