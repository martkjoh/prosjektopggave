...
% This is achieved with the configurations
% \begin{eqnarray}
%     \bar q_c q_c, \quad \varepsilon^{c_1 c_2 c_3} q_{c_1} q_{c_2} q_{c_3},
%     \quad
%     \varepsilon^{c_1 c_2 c_3} \bar q_{c_1} \bar q_{c_2} \bar q_{c_3},
% \end{eqnarray}
% which are the mesons, baryons and anti-baryons.
...
% All unitary matrices $U$ can be written as $e^{i\theta} U'$, where $\theta$ is a real number, and $U'$ is a matrix with determinant 1, which means we can decompose the Lie group into .
...
\begin{equation}
    U_V 
    % =\exp{i \eta_\alpha^V T_\alpha^L} \exp{i \eta_\alpha^V T_\alpha^R}
    = \exp{i \eta_\alpha^V T_\alpha^V \one}.
\end{equation}
% The remaining set of transformations,
