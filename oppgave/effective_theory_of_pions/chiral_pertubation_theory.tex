...
This section is based on \cite{Schwartz:QFT,weinberg_1996_vol2,Scherer2002IntroductionTC}, in addition to the original work~\cite{Gasser-Leutwyler:chiral,WeinbergPhenom,Scherer:PhysRevD.53.315}

In this paper, we will consider the interaction of the two lightest quarks, the up and down quarks $u$ and $d$, i.e., $N_f = 2$.
In this case, the symmetry group of rotations in the flavor indices is $G_f = U(1) \times \SU(2)_L \times \SU(2)_R$.
...
The $G_f$ symmetry is \emph{explicitly} broken.
...
\end{equation}
%
The Lagrangian containing external terms is
\begin{align}
    \Ell_\text{ext} 
    & = 
...
\end{align}
%
Here, $v^\mu_a, v_{(s)}^\mu, a^\mu_a, s_a, s_0$ and $p_a$ are the external source currents, and we denote
\begin{equation}
...
Therefore, all terms in the effective Lagrangian will be proportional to a certain number of derivatives of the pion fields, $\partial_\mu \pi$, which Lorentz invariance demands to be even.
...
\end{equation}
%
Du to how $G$ factors into two Lie groups, the constituents of the Mauer-Cartan form are
\begin{equation}
...
As $d_\mu \in \liea{su}{2}$, which we represent by the traceless Pauli matrices, we have
\begin{equation}
    \Tr{d_\mu} = 0.
\end{equation}
%
...
By this definition, the form of the covariant derivative is decided by what it acts on.
...
For invariant quantities under $\SU(2)_L\times \SU(2)_R$, the covariant derivative is the ordinary partial derivative.
In our case, $a_\mu = 0$, and the covariant derivative is
\begin{equation}
...
We are interested in systems with non-zero isospin, 
\begin{equation}
    Q_I = \int \dd^3 x \, V^0_3 = \int \dd^3 x \, \frac{1}{2}  \bar q \gamma^0 \tau_3 q,
\end{equation}
%
and a corresponding non-zero isospin chemical potential $\mu_I$.
...
As in the case of Yang-Mills theory, we may form a field strength tensor to create invariant terms,
\begin{align}
    f_{\mu\nu}^R &= \partial_\mu r_\nu - \partial_\nu r_\mu - i [r_\mu, r_\nu], \\
    f_{\mu\nu}^L &= \partial_\mu l_\nu - \partial_\nu l_\mu - i [l_\mu, l_\nu].
\end{align}
