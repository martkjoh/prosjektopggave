...
\todo{Make sure the sign is right w.r.t the charge}\footnote{An unfortunate notational convention is that $E_+$ is the energy of $\pi_-$-particle, and $E_+$ for the $\pi_+$-particle. This is because the positivley charged pion, $\pi_+$, has isospin $I_3 = +1$, so that the mass will decrease as $\mu_I$ increseas, and hence the negative sign.}
