...
In imaginary time formalism for thermal field theory, which is described in \autoref{section:thermal field theory}, we find that the free energy density is given by 
\begin{equation}
    \Ef = - \frac{1}{V \beta} \ln Z.
\end{equation}
Here, $Z$ is the partition function, and $V$ the volume of space.
In the low temperature limit $\beta \rightarrow \infty$, the partition function is related to vacuum transition amplitude $Z_0 = Z[J=0]$, as described in \autoref{section:path integral}, by a Wick rotation.
...
\begin{align*}
    ...
    = 0.
\end{align*}
This equation defines the relationship between the chemical potential $\mu_I$, and the ground state parameter $\alpha$.
...
\begin{figure}[ht]
    \centering
    % \vspace{-1cm}
    \includegraphics{figurer/numerics/free_energy_surface_crop.pdf}
...
\end{equation}
For $|\vv p|^2 \gg m^2$, this term is asymptotic to $|\vv p|^{-3}$, so it retains its UV behavior.
...
\begin{align}
    ...
    % = (2 B_0 m) \cos \alpha - \mu_i^2(\cos 2 \alpha - \cos^2 \alpha)
