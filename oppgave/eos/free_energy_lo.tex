...
In \autoref{section: effective action}, we found an explicit formula for this to one-loop order, \cref{effective potential}.
...
The linear part of the classical potential is given by \cref{L1} to leading order. It reads $\Ve^{(1)} = f(\mu_I{}^2\cos{\alpha} - \bar m^2)\sin{\alpha} \, \pi_1 $, and vanishes given the minimizition criterion \cref{leading order minization}.
\begin{figure}[ht]
    \centering
    \includegraphics{figurer/numerics/free_energy_surface.pdf}
    \caption{The free energy surface as a function of $\mu_I$ and $\alpha$. $\alpha$ is the found by minimizing $\Ef$ for a given $\mu_I$. This leads to a curve across the free energy surface, as show in the plot.}
...
\end{equation}
We see that the result is what we would expect physically; the total energy is integral to each mode's energy.
