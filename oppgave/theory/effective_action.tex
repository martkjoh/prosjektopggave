...
If we add a new loop to a diagram with $n$ loops, the formula still holds by joining two vertices.
...
\begin{equation}
    ...
    C \det(- \fdiff{ \Gamma[\varphi_J]}{\varphi(x), \varphi(y)})
...
After we remove the constant, this term proportional to $g^0$, i.e. it is made up of one-loop terms.
...
\begin{align}
    ...
    \right) 
\end{align}
If we expand the expectation value a funcional of $\varphi$ as a powerseries, each term can be rewritten as functional derivatives of $Z[J]$,
\begin{equation}
    \ex{f[\varphi]}_0 = f\left[-i\fdiff{}{J}\right] Z_0[J] \big|_{J=0}.
\end{equation}
This is a generalization of the result that correlation functions are functional derivatives of the generating functional.
...
We know that $S_I$ is made up of terms of the third power or higher in the fields.
...
This shows that the last logarithm is at least $\Oh{g}$, and thus that the firs contains \emph{all} one loop contributions to $\ln Z$.
The effective potential to 1 loop order is therefore
\begin{equation}
