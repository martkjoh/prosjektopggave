In this section, we will survey some general properties of quantum field theory that are necessary for chiral perturbation theory.
...
In the path integral formalism, one evaluates quantum observable by summing or integrating all possible configurations in space and time.
...
\end{equation}
The terms in this series are represented by Feynman diagrams, which are constructed using the Feynman rules, and can be read from the action.
...
\begin{align}
    ...
    \times \dots.
\end{align}
If sub-diagram $i$ as a stand-alone diagram equals $V_i$, each copy of that subdiagram contributes a factor $V_i$ to the total diagram.
