...
The linear sigma model has $N$ real scalar $\varphi_i$ fields which are governed by the Lagrangian
\begin{equation}
    \Ell[\varphi] 
...
\begin{equation}
    ...
    \quad O^{-1} = O^{T}.
\end{equation}
This is a global, internal and continuous set of transformations, with the infinitesimal form, exactly as discussed earlier.
The set of all such transformations form the Lie group $O(N)$, which we will define later.
...
For symmetry transformations, the group multiplication is composition.
This map is closed, as two symmetry transformations are another transformation, and the identity map is a symmetry transformation.
...
\end{equation}
The set of generators thus form a vector space, called the tangent space of the identity element, $T_\one G$, and the coordinates $\eta_a$ induce a coordinate basis.
...
% \end{equation}
% If we assume $g(\eta=0) = \one$, then we must have $f_a(\eta, 0) = f_a(0, \eta) = \eta_a$.
...
For connected and compact Lie groups, all elements can be written on the form
\begin{equation}
...
This is illustrated in \autoref{fig:Mexican hat}.
...
\end{equation}
With the assumption that the groudnstate is constant, we get 
\begin{equation}
    \diffp{\Veff}{\varphi_k, \varphi_i} \, t_{i j} \ex{\varphi_j}_0 = 0.
\end{equation}
This is trival for unbroken symmetries, as $t_{ik}\ex{\varphi}_0 = 0$ by definition.
However, in the case of a broken symmetry, the second derivative of the effective potentail has an eigenvector $t^\alpha_{ij} \ex{\varphi_j}$ with as zero eigenvalue for each broken generator.
...
In Lorentz-invariant systems, each zero-mas vector corresponds to a masses particles, called a Goldstone boson.\footnote{ The particles are bosons due to the bosonic nature of the transformations, $t$. If the generators are Grassmann numbers, the resulting particle, called a goldstinos, are fermions.}
...
Any excitations in the rational direction, however, does not cost any energy, which is indicative of a massless mode.
...
If we instead consider the $N=3$ linear sigma model, which has a three-dimensional symmetry group, rotations of the sphere, we see that the ground state invariant is left invariant under subgroup of the original symmetry transformations.
The ground state manifold of this system, the set of all the degenerate ground states, is then a sphere.
