...
\begin{align}
    ...
    \times \dots.
\end{align}
If the value of subdiagram $i$ is $V_i$, then each copy of that subdiagram contribute a factor $V_i$ to the value of the total diagram.
...
\begin{equation}
    ...
    = \exp({\sum}_i V_i).
\end{equation}
We showed that the generating functional $Z[J]$ were the $Z_0[0]$ times the sum of all diagrams due to external currents.
...
% \end{equation}
% In this case, however, we can not assume that only the second derivatives remain, as the case was for the free theory.
