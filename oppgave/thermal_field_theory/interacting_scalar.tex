...
\end{align*}
where $X_i$ for $i>m$ is defined as $X_j$, where $j = i \mod m$, such that $X_{m + i} = X_i$.
The functions $a,\,b$ represents a possible pairing, as described in \autoref{section: effective action}.
...
\begin{align}
    ...
    & = \frac{1}{\beta} D_0(K).
\end{align}
Lastly, one has to divide by a symmetry factor of the diagram $s$, which is described in detail in~\cite{Peskin:IntroQFT}.
